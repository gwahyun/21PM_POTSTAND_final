...
public class BookDbTest {
	...
	public ModelAndView bookSetting(ModelAndView mv, HttpServletRequest request) throws Exception {
		...
		for (int i = 0; i < cateList.size(); i++) {
			...
			URL url = new URL("https://openapi.naver.com/v1/search/book_adv.xml?&display=80&start=1&d_catg="
		...
		// api 키값 설정
			...
			try {
				...
				for (int j = 0; j < nodeList.getLength(); j++) {
					...
					count+=session.insert("order.insertBook", b);
	...
	public List getCategoryList() {
		List<Sort> cateList = session.selectList("order.selectSortAll");
		return cateList;
	}
